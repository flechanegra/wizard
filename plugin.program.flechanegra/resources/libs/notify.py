...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "Atualmente voce nao possui nenhuma Build Instalada usando o %s.\n\nPara instalar, proceda da seguinte maneira: selecione 'IGNORAR', va em ADDONS-->PROGRAM ADD-ONS e execute o assistente  Flecha Negra WIZARD, e siga as instrucoes para instalar uma versao da %s\n\nObrigado por utilizar o %s." % (ADDONTITLE,BUILDERNAME, ADDONTITLE)
		...
		def showdialog(self):
			self.getControl(self.image).setImage('000000')
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			#self.fanart = kwargs['fanart']
			self.msgupdate  = "Existe uma nova versao da:\n[COLOR %s]%s[/COLOR]\n\nVersao Instalada: v[COLOR %s]%s[/COLOR]\nNova versao disponivel: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Recomendamos fortemente que ao atualizar voce utilize a opcao ISTALACAO LIMPA[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
			self.msgcurrent = "Executando a Build disponivel mais recente:\n[COLOR %s]%s[/COLOR]\n\nVersao Instalada: v[COLOR %s]%s[/COLOR]\nNova versao disponivel: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Recomendamos fortemente que ao atualizar voce utilize a opcao ISTALACAO LIMPA[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
